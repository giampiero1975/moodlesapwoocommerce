 ...
 * * 1. PayPal: Recupera transazioni. Se � un ordine WC -> Moodle. Se NO -> Tabella 'results' (RIPRISTINATO).
...
if (isset($log)) {
    ...
    $log->warning('Modalit� Applicazione: ' . (defined('APP_MODE') ? APP_MODE : 'NON DEFINITA!'));
}
...
$startDatePayPal = '2026-06-14T00:00:00Z';
...
function parseWooInvoiceId(string $invoiceId): ?array
{
    $invoiceId = trim($invoiceId);
    if (preg_match('/^\{?([A-Za-z0-9]+)\}?\s*[-_\/\s]*\s*(\d+)$/', $invoiceId, $matches)) {
...
foreach ($receivedPayments as $tx) {
    ...
    // Identifica se � un ordine WooCommerce
    $wooOrderId = null;
    $instanceConfig = null;
    
    if (!empty($paypal_invoice_id)) {

    $parsedInvoice = parseWooInvoiceId($paypal_invoice_id);
    ...
    if ($parsedInvoice) {
        ...
        foreach ($wcInstanceMapping as $prefix => $config) {
            ...
            if (strcasecmp($parsedInvoice['prefix'], $cleanPrefix) === 0) {

                $wooOrderId = $parsedInvoice['order_id'];
                ...
                if (isset($log)) {
                    $log->warning(
                        "DEBUG MATCH REGEX: Invoice '{$paypal_invoice_id}' -> Prefisso '{$parsedInvoice['prefix']}' -> Ordine {$wooOrderId}"
                    );
                }
    ...
    if ($instanceConfig && $wooOrderId) {
        ...
        try {
            if (!checkWooOrderAlreadyQueued($wooOrderId, $instanceConfig['moodle_db_name'] ?? null)) {
                
                // *** FIX IMPORTANTE: Definiamo che questo � un flusso PayPal ***
                ...
                if (isset($log)) $log->info("PayPal: Ordine $wooOrderId gi� processato. Salto.");
            }
        } catch (Exception $e) {
        ...
        // CASO B: NON � WOOCOMMERCE -> SALVA IN RESULTS
        ...
        try {
            ...
            if (!$exists && defined('APP_MODE') && APP_MODE === 'PRODUCTION') {
                ...
                if ($executed) {
                    if (isset($log)) {
                        $log->info("Salvato in 'results' OK.");
                    }
                    ...
                    if (isset($log)) {
                        $log->error("Errore insert 'results': " . $stmt->error);
                    }
                }
                $stmt->close();
            }
...
foreach ($wcInstanceMapping as $prefix => $config) {
    ...
    foreach ($bacsOrderIds as $wooOrderId) {
        ...
        // Controllo se gi� processato
        if (checkWooOrderAlreadyQueued((string)$wooOrderId, $currentConfig['moodle_db_name'] ?? null)) {
            continue;
        }
