...
class SapModelEs extends dbsapEs
{
    ...
    public function setState($fattProv){
        $logger = Logger::get_logger();
        $logger->log("Provincia Moodle: ". $fattProv);
        $sql = "SELECT Code from OCST WHERE ISNUMERIC(code)=0 AND country='ES' and name like '".$fattProv."'";
        $logger->log($sql);
        $nextDocNum = $this->select($sql);
        return $nextDocNum[0]['Code'];
    }
    ...
    public function setCardCode(){
        $logger = Logger::get_logger();
        $sql = "SELECT (max(SUBSTRING ( cardcode ,3 , len(cardname) ))+1) as 'maxCardCode' FROM OCRD WHERE cardcode LIKE 'CM%';";
        $logger->log($sql);
    ...
    public function getUsers($cf)
    {
        ...
        $sql ="SELECT TOP 1 OCRD.cardcode, OCRD.cardname, OCRD.AddId, OCRD.LicTradNum AS 'partitaiva', IPACodePA, E_Mail, PECAddr, ShipToDef, BillToDef, pymcode FROM OCRD WHERE OCRD.AddId = '".$this->cf."'  ORDER BY cardcode desc;";
        $logger->log("*** GET anagrafica da OCRD: ".$sql);
        $getRegistry = $this->select($sql);
        $getRegistry = mb_convert_encoding($getRegistry, 'ISO-8859-1','UTF-8');
        /*        
        $logger->log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
        $logger->log("Debug caratteri speciali");
        $logger->dump($getRegistry);
        $logger->log("[1] mb_detect_encoding: ". mb_detect_encoding($getRegistry['0']['cardname'], "auto"));

        $logger->log(">>>>>>>>> CONVERTO >>>>>>>>>");
        $getRegistry = mb_convert_encoding($getRegistry, 'ISO-8859-1','UTF-8');        
        $logger->dump($getRegistry);
        
        $logger->log("[1] mb_detect_encoding: ". mb_detect_encoding($getRegistry['0']['cardname']));
        $logger->log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
        */
        ...
        $getBillingAddress = mb_convert_encoding($getBillingAddress, 'ISO-8859-1', 'UTF-8');
        #$logger->log("[1] mb_detect_encoding: ". mb_detect_encoding($getBillingAddress['0']['Address'], "auto"));
        #$logger->log("[1] mb_detect_encoding: ". mb_detect_encoding($getBillingAddress['0']['Address']));
        #$logger->dump($getBillingAddress);
        #$getRegistry = mb_convert_encoding($getRegistry, 'UTF-8','ISO-8859-1');
        #$logger->dump($getBillingAddress);
        ...
        #$logger->log("mb_convert_encoding: ".mb_convert_encoding($getRegistry, 'ISO-8859-1', 'UTF-8'));
        #$getRegistry = mb_convert_encoding($getRegistry, 'UTF-8','ISO-8859-1');
        $logger->log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
        ...
        */
        ...
        $getShippingAddress = mb_convert_encoding($getShippingAddress, 'ISO-8859-1', 'UTF-8');
        
        if(empty($getShippingAddress)){
            $logger->log("Indirizzo di spedizione mancante, procedo con la duplica indirizzo di Billing");
            $sql= "INSERT INTO CRD1 (Address, CardCode, Street, Block, ZipCode, City, County, Country, State, UserSign, LogInstanc, ObjType, LicTradNum, LineNum, TaxCode, Building, Address2, Address3, AddrType, StreetNo, AltCrdName, AltTaxId, TaxOffice, GlblLocNum, Ntnlty, DIOTNat, TaaSEnbl, GSTRegnNo, GSTType, CreateDate, CreateTS, EncryptIV, MYFType, VatResDate, VatResCode, VatResName, VatResAddr, U_RTS_Latitude, U_RTS_Longitude, AdresType) ".
                  "SELECT Address, CardCode, Street, Block, ZipCode, City, County, Country, State, UserSign, LogInstanc, ObjType, LicTradNum, LineNum, TaxCode, Building, Address2, Address3, AddrType, StreetNo, AltCrdName, AltTaxId, TaxOffice, GlblLocNum, Ntnlty, DIOTNat, TaaSEnbl, GSTRegnNo, GSTType, CreateDate, CreateTS, EncryptIV, MYFType, VatResDate, VatResCode, VatResName, VatResAddr, U_RTS_Latitude, U_RTS_Longitude,'S' AS AdresType from CRD1 WHERE cardcode = '".$getRegistry['0']['cardcode']."'";
            $logger->log($sql);
            if(!$this->updCrm($sql))
                return false;
        ...
        */
        ...
        $businesspatner = [
            ...
            // fatturazione
            'Name' => $getBillingAddress['0']['Address'],
    ...
    public function getItem($item) {
        ...
        $sql = "SELECT trim(OITM.itemcode) as itemcode, OITM.itemname, OITM.vatgourpSa, OITW.RevenuesAc  "
                ." FROM OITM JOIN OITW ON OITM.ItemCode = OITW.ItemCode"
                ." WHERE OITW.itemcode ='".$this->item."';";
        
        $logger->log($sql);
        $datiarticolo = $this->select($sql);
        $datiarticolo = mb_convert_encoding($datiarticolo, 'ISO-8859-1', 'UTF-8');
    ...
    public function setAlign($cardcode, $allineamento){
        ...
        if (array_key_exists('U', $allineamento)) {
            ...
            foreach ($allineamento['U'] as $key => $value){
                ...
                if(array_key_last($allineamento["U"])!=$key) $sql .=", ";
            }
        ...
        if (array_key_exists('B', $allineamento)) {
            if (array_key_exists('B', $allineamento)) {
                ...
                foreach ($allineamento['B'] as $key => $value){
                    $sql .= $key ." = '".$this->checkstring($value)."'";
                    if(array_key_last($allineamento["B"])!=$key) $sql .=", ";
                }
                
                $sql .= " where cardcode='".$cardcode."' and AdresType='B';";
                $logger->log($sql);
                if(!$this->updCrm($sql))
                    $logger->log("Errore aggiornamento indirizzo fatturazione");
                    $logger->log($sql);
                    return false;
            }
        }
        ...
        if (array_key_exists('S', $allineamento)) {
            if (array_key_exists('S', $allineamento)) {
                ...
                foreach ($allineamento['S'] as $key => $value){
                    if(array_key_last($allineamento["S"])!=$key) $sql .=", ";
                }
